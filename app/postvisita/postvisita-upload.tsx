...
export default function PostVisitaUpload() {
  ...
  async function seleccionarArchivo(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const encontrados: string[] = []; const repetidos = new Set<string>();
      for (let i = filaCabecera + 1; i < filas.length; i += 1) {
        ...
        repetidos.add(limpio); encontrados.push(limpio);
      }
  ...
  return <div className="space-y-6">
    ...
    {clientes.length > 0 && <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white"><div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-5 py-4"><div><p className="text-xs font-black uppercase tracking-wider text-blue-700">Lectura correcta</p><h3 className="text-lg font-black text-slate-800">{clientes.length} clientes detectados</h3></div><span className="rounded-full bg-emerald-100 px-3 py-1 text-sm font-black text-emerald-700">OK</span></div><div className="max-h-80 overflow-auto"><table className="w-full text-left text-sm"><thead className="sticky top-0 bg-white text-xs uppercase text-slate-500 shadow-sm"><tr><th className="px-5 py-3">#</th><th className="px-5 py-3">Nº de cliente</th></tr></thead><tbody className="divide-y divide-slate-100">{clientes.map((cliente, index) => <tr key={`${cliente}-${index}`}><td className="px-5 py-3 text-slate-400">{index + 1}</td><td className="px-5 py-3 font-bold text-slate-800">{cliente}</td></tr>)}</tbody></table></div></section>}
  </div>;
}
